...
import { Settings, Wrench, Fuel, Plus, AlertCircle, CheckCircle2, X, Loader2, Trash2, Edit2, Droplets, DollarSign, Calendar, Info, Clock } from 'lucide-react';
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmitReparacion} className="p-6 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5"><label className="text-xs font-bold text-slate-600">Costo ($)</label><CurrencyInput placeholder="Ej. 15000" value={repCosto} onChange={(val) => setRepCosto(val)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F3160]/20" /></div>
              </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            <form onSubmit={handleSubmitCombustible} className="p-6 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex flex-col gap-1.5"><label className="text-xs font-bold text-slate-600">Litros</label><CurrencyInput prefix="" placeholder="Ej. 45.5" value={combLitros} onChange={(val) => setCombLitros(val)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F3160]/20" /></div>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-1.5"><label className="text-xs font-bold text-slate-600">Costo Total ($)</label><CurrencyInput placeholder="Ej. 35000" value={combCosto} onChange={(val) => setCombCosto(val)} required className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F3160]/20" /></div>
                <div className="flex flex-col gap-1.5"><label className="text-xs font-bold text-slate-600">Precio x Litro (Opcional)</label><CurrencyInput placeholder="Ej. 750" value={combPrecio} onChange={(val) => setCombPrecio(val)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#0F3160]/20" /></div>
              </div>
