...
import { X, Check, Search, Tag, Loader2, Image as ImageIcon } from 'lucide-react';

interface Props {
  initialData: { nombre: string; codigo_barra: string; imagen_url?: string; marca?: string; found: boolean };
  onConfirm: (data: { nombre: string; categoria?: string; codigo_barra?: string; imagen_url?: string }) => Promise<void>;
  onCancel: () => void;
}
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-extrabold text-lg text-[#0F3160]">
            {initialData.found ? 'Confirmar Producto' : 'Producto Nuevo'}
          </h3>
